...
import { postPsicologo } from "../../../psicologos/services/psicologo.service.jsx";
...
import { getMateriaForPsicologo } from "../../../materias/services/materia.service.jsx"; // endpoint nuevo
...
const allowedMateriaIds = new Set(["6", "7"]);
const isAllowedMateria = (m) => {
  if (!m) return false;
  if (allowedMateriaIds.has(String(m.idmateria))) return true;
  const nombre = (m.nombre || "").toString().toLowerCase();
  return nombre.includes("filosof") || nombre.includes("psicolog");
};
...
export default function FormularioCreacionPs() {
  ...
  const validate = () => {
    ...
    if (v.idmateria) {
      ...
      if (!isAllowedMateria(m)) errs.idmateria = "Materia no permitida.";
    }
